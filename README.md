...
```bash
# Seguridad JWT
JWT_SECRET="cambiar-por-clave-super-secreta-de-produccion"
JWT_EXPIRATION="86400000"

# Base de datos (cambiar a PostgreSQL/MySQL)
SPRING_DATASOURCE_URL="jdbc:postgresql://host:5432/nissum_db"
SPRING_DATASOURCE_USERNAME="nissum_user"
...
- **JVM Memory**: Uso de memoria heap/non-heap
- **Database Connections**: Pool de conexiones activas
- **HTTP Requests**: Latencia y throughput de endpoints
- **Custom Metrics**: Registros de usuarios, tokens generados
...
### ❌ Error 500 en Swagger UI - "NoSuchMethodError: ControllerAdviceBean"

**🚨 Problema**: Error 500 al acceder a `/swagger-ui/index.html` o `/v3/api-docs` con mensaje:
```
NoSuchMethodError: 'void org.springframework.web.method.ControllerAdviceBean.<init>(java.lang.Object)'
```

**🔍 Causa**: Incompatibilidad entre `@RestControllerAdvice` y Spring Boot 3.5.7 debido a cambios en la inicialización de beans de Spring Framework.

**✅ Solución Implementada**:
1. **SpringDoc OpenAPI**: Downgrade a versión 2.0.4 (compatible con Spring Boot 3.5.7)
2. **Error Handling**: Reemplazar `@RestControllerAdvice` con arquitectura alternativa:
   - `ErrorHandlingService`: Servicio centralizado para manejo de errores
   - `CustomErrorController`: Controller específico para errores HTTP estándar
3. **Configuración**: Expandir endpoints públicos en `SecurityConfig` para recursos Swagger

### 🔐 Error 403 - Forbidden en Endpoints Protegidos

**🚨 Problema**: Error 403 al acceder a endpoints que requieren autenticación.

**✅ Solución**:
1. **Obtener Token**: Primero registrar usuario en `/api/users/register`
2. **Configurar Header**: Usar el token JWT en header `Authorization: Bearer <token>`
3. **Swagger UI**: Usar botón "Authorize" para configurar token globalmente

### 🗄️ Error de Conexión H2 Database

**🚨 Problema**: No se puede conectar a H2 console o base de datos no inicializada.

**✅ Solución**:
1. Verificar que la aplicación esté ejecutándose en puerto 8080
...
### 🧪 Tests Fallando - Errores de Mock

**🚨 Problema**: Tests unitarios fallan por errores de mock después de cambios en arquitectura.

**✅ Solución**:
1. Actualizar mocks para incluir `ErrorHandlingService`
2. Verificar que todos los servicios dependientes estén mockeados
3. Ejecutar: `./mvnw clean test` para rebuild completo

### 🔑 JWT Token Expirado

**🚨 Problema**: Error 401 - Token JWT ha expirado.

**✅ Solución**:
1. **Desarrollo**: Token válido por 24 horas (86400000 ms)
2. **Testing**: Token válido por 1 hora (3600000 ms)
3. **Regenerar**: Hacer nuevo POST a `/api/users/register` con usuario único
